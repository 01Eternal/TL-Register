...
export class ModHooks {
    ...
    static initialize() {
        ...
        Terraria.WorldGen.KillTile.hook(
    ...
    let ShootDubble1 = [];
    for (let i = 0; i < 50; i++) {
    ShootDubble1[i] = Terraria.Main.GameUpdateCount;
     }
    if (item.type === 2059 && Terraria.Main.GameUpdateCount - ShootDubble1[slot] > 1) {
      ShootDubble1[slot] = Terraria.Main.GameUpdateCount;
      const Shoot1IDs = [645, 709, 1];
      const randomShoot1ID = Shoot1IDs[Math.floor(Math.random() * Shoot1IDs.length)];
      item.shoot = randomShoot1ID;
    } }
...
for (let i2 = 0; i2 < 50; i2++) {
  ShootDubble2[i2] = {count: Terraria.Main.GameUpdateCount, index: 0, isUsing: false, lastUseTime: -1};
}

for (let slot2 = 0; slot2 < 50; slot2++) {
    const item2 = self.inventory[slot2];
    
    if (item2.type === ModItem.getTypeByName('EvilSmasher')) {
      if (self.itemAnimation > 0 && !ShootDubble2[slot2].isUsing) {
        ShootDubble2[slot2].isUsing = true;
        ShootDubble2[slot2].index = 0;
      } else if (self.itemAnimation == 0) {
        ShootDubble2[slot2].isUsing = false;
      }

      if (ShootDubble2[slot2].isUsing && Terraria.Main.GameUpdateCount - ShootDubble2[slot2].count > 200) {
        ShootDubble2[slot2].count = Terraria.Main.GameUpdateCount;
        
        const damageValues = [10, 30, 50, 80, 120];
        const nextDamageValue = damageValues[ShootDubble2[slot2].index];
        
        if (item2.damage !== nextDamageValue) {
          item2.damage = nextDamageValue;
          ShootDubble2[slot2].lastUseTime = nextDamageValue;
        }

        if (ShootDubble2[slot2].index < damageValues.length - 1) {
          ShootDubble2[slot2].index++; }
         }
        ...
        Terraria.Item.Prefix.hook((original, self, pre) => {
            ...
            let unifiedRandom = Terraria.WorldGen
            ...
            while (flag) {
                ...
                if (
                    pre === -1 &&
                    unifiedRandom["int Next(int maxValue)"](4) === 0
                ) {
                    prefix = 0;
                }
                ...
                if (prefix === -1 || prefix === -2 || prefix === -3) {
                    const modPrefix = ItemLoader.ChoosePrefix(
                        self,
                        unifiedRandom
                    );
                    if (modPrefix >= 0) {
                        ...
                        let meleeRandom =
                            unifiedRandom["int Next(int maxValue)"](40);
                        switch (meleeRandom) {
                            case 0:
                                prefix = 1;
                                break;
                        ...
                        let weaponRandom =
                            unifiedRandom["int Next(int maxValue)"](14);
                        switch (weaponRandom) {
                            case 0:
                                prefix = 36;
                                break;
                        ...
                        let rangedRandom =
                            unifiedRandom["int Next(int maxValue)"](35);
                        switch (rangedRandom) {
                            case 0:
                                prefix = 16;
                                break;
                        ...
                        let magicRandom =
                            unifiedRandom["int Next(int maxValue)"](36);
                        switch (magicRandom) {
                            case 0:
                                prefix = 26;
                                break;
                ...
                if (
                    damage != 1.0 &&
                    Math.round(self.damage * damage) === self.damage
                ) {
                    flag = true;
                    prefix = -1;
                }
                if (
                    animation != 1.0 &&
                    Math.round(self.useAnimation * animation) ===
                        self.useAnimation
                ) {
                    flag = true;
                    prefix = -1;
        ...
        /*Terraria.Item['void SetDefaults(int Type, bool noMatCheck)'].hook((original, self, type, noMatCheck, variant) => {
        ...
        Terraria.Item[
            "void SetDefaults(int Type, bool noMatCheck, ItemVariant variant)"
        ...
        Terraria.Utils[
            "Rectangle Frame(Texture2D tex, int horizontalFrames, int verticalFrames, int frameX, int frameY, int sizeOffsetX, int sizeOffsetY)"
        ].hook(
            (
                original,
                tex,
                ...
                const textureOverrideInfo =
                    ModTexture.overrideFrames[tex._sourceLoadAsset];
                if (
                    textureOverrideInfo != undefined &&
                    textureOverrideInfo != -1
                ) {
        ...
        Terraria.Item.GetDrawHitbox.hook((original, type, user) => {
            if (type >= ItemLoader.MAX_VANILLA_ID) {
                const texture =
                    Terraria.GameContent.TextureAssets.Item[type].Value;
        ...
        Terraria.Player.ApplyEquipFunctional.hook(
        ...
        Terraria.Player.ApplyEquipVanity.hook(
            (original, self, itemSlot, currentItem) => {
                original(self, itemSlot, currentItem);
        ...
        Terraria.Player.ApplyNPCOnHitEffects.hook(
            ( original,  self,  sItem,   itemRectangle,    damage,    knockBack,    npcIndex,    dmgRandomized,   dmgDone) => {
                original( self,   sItem,   itemRectangle,     damage,     knockBack,     npcIndex,    dmgRandomized,    dmgDone
                );

                ItemLoader.OnHitNPC || GlobalItem(
                    sItem,
                    self,
                    Terraria.Main.npc[npcIndex],
                    dmgDone,
                    knockBack,
                    ModHooks.OnHitTemp[npcIndex].crit
                );
                ModHooks.OnHitTemp = {};
            }
        );
        ...
        Terraria.Main.AnglerQuestSwap.hook(original => {
            ...
            while (flag2) {
                flag2 = false;
                Terraria.Main.anglerQuest = Terraria.Main.rand[
                    "int Next(int maxValue)"
                ](Terraria.Main.anglerQuestItemNetIDs.length);
                const num =
                    Terraria.Main.anglerQuestItemNetIDs[
                        Terraria.Main.anglerQuest
                    ];
        ...
        Terraria.Lang.AnglerQuestChat.hook((original, turnIn) => {
            ...
            const turnInText = Terraria.Localization.Language.SelectRandom(
                Terraria.Lang[
                    "LanguageSearchFilter CreateDialogFilter(string startsWith, object substitutions)"
                ]("AnglerQuestText.TurnIn_", obj),
                null
            )["ng FormatWith(object obj)"](obj);
            if (turnIn) {
                return turnInText;
            }
            ...
            const id = (Terraria.Main.npcChatCornerItem =
                Terraria.Main.anglerQuestItemNetIDs[Terraria.Main.anglerQuest]);
            const modQuestText = ItemLoader.AnglerChat(id);
            const vanillaQuestText = Terraria.Localization.Language[
                "string GetTextValueWith(string key, object obj)"
            ](
                "AnglerQuestText.Quest_" +
                    Terraria.ID.ItemID.Search.GetName(id),
                obj
            );
        ...
        Terraria.Player.ItemCheck_ApplyUseStyle.hook(
        ...
        Terraria.Player.ItemCheck_Shoot.hook(
            (original, self, i, sItem, weaponDamage) => {
                if (!CombinedLoader.CanShoot(self, sItem)) {
                    return;
                }

                const pointPosition = self.RotatedRelativePoint(
                    self.MountedCenter,
                    true,
                    true
                );
                ...
                velocity.X =
                    ...
                    Terraria.Main.screenPosition.X -
                    pointPosition.X;
                velocity.Y =
                    Terraria.Main.mouseY +
                    Terraria.Main.screenPosition.Y -
                    pointPosition.Y;
                if (
                    !CombinedLoader.Shoot(
                        self,
                        sItem,
                        pointPosition,
                        velocity,
        ...
        Terraria.Player.ItemCheck_CheckCanUse.hook((original, self, sItem) => {
            ...
            if (
                item.type < ItemLoader.MAX_VANILLA_ID ||
                CombinedLoader.CanUseItem(self, sItem)
            ) {
        ...
        Terraria.Player.ItemCheck.hook((original, self) => {
            ...
            if (item.type >= ItemLoader.MAX_VANILLA_ID) {
                ...
                if (self.whoAmI === Terraria.Main.myPlayer) {
                    if (
                        self.itemTimeMax !== 0 &&
                        self.itemTime === self.itemTimeMax &&
                        item.consumable
                    ) {
        ...
        Terraria.Player.QuickMount_GetItemToUse.hook((original, self) => {
            ...
            if (
                (mountEquip.type < ItemLoader.MAX_VANILLA_ID ||
                    (mountEquip.type >= ItemLoader.MAX_VANILLA_ID &&
                        CombinedLoader.CanUseItem(self, mountEquip))) &&
                ...
                for (let i = 0; i < 58; i++) {
                    ...
                    if (
                        (inventory.type < ItemLoader.MAX_VANILLA_ID ||
                            (inventory.type >= ItemLoader.MAX_VANILLA_ID &&
                                CombinedLoader.CanUseItem(self, inventory))) &&
        ...
        Terraria.Player.QuickMount.hook((original, self) => {
            if (self.mount.Active) {
                ...
                if (
                    item !== null &&
                    item.mountType !== -1 &&
                    Terraria.Mount.CanMount(self.mount, item.mountType, self)
                ) {
                    if (!self.QuickMinecartSnap()) {
                        Terraria.Mount.SetMount(
                            self.mount,
                            item.mountType,
                            self,
                            false
                        ...
                        if (item.UseSound !== null) {
                            Terraria.Audio.SoundEngine[
                                "SoundEffectInstance PlaySound(LegacySoundStyle type, Vector2 position)"
        ...
        Terraria.Player.ItemCheck_ApplyManaRegenDelay.hook(
            (original, self, sItem) => {
        ...
        Terraria.Player.ItemCheck_PayMana.hook(
            (original, self, sItem, canUse) => {
                ...
                if (
                    sItem.shoot > 0 &&
                    Terraria.ID.ProjectileID.Sets.TurretFeature[sItem.shoot] &&
                    flag
                ) {
                    flag2 = true;
                }
                ...
                if (
                    sItem.shoot > 0 &&
                    Terraria.ID.ProjectileID.Sets.MinionTargettingFeature[
                        sItem.shoot
                    ] &&
                    flag
                ) {
                ...
                if (
                    sItem.type !== 3269 &&
                    !CombinedLoader.CheckMana(sItem, self, -1, !flag2)
                ) {
                    canUse = false;
                }
        ...
        Terraria.Player.GetDyeTraderReward.hook((original, self) => {
            ...
            const num =
                rewardPool[
                    Terraria.Main.rand["int Next(int maxValue)"](
                        rewardPool.length
                    )
                ];
            ...
            const item2 = self.GetItem(
                self.whoAmI,
                item,
                Terraria.GetItemSettings.NPCEntityToPlayerInventorySettings,
                false
            );
            if (item2.stack > 0) {
                Terraria.Item[
